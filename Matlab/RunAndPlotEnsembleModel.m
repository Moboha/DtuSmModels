...
pathToPrmFile = 'H:\SyncPC\csharp\DtuSmModels\TestData\DAtests\SlowAndFast.prm';
...
xxxhandle = myDtuSmObj.AssemblyHandle.GetType('DtuSmModels.SmOutput+OutputType');
...
for i=1:N
   ...
   ensmodel.StepModels();
   truth_model.stepModelWithSetRain();
   obs = conns.Item(index1).getFlow();  
   
    %ensmodel.Mat_perturbeParameters(0.0001,2);%(std, truncationLimInStd)
    if(~mod(i,1)) %only update params every xth step
        ensmodel.Mat_updateParamsToObs(obs, 0.001^2, 1.0, 1.0);%(value, var, inflationFactor)
    end
    
    ensmodel.Mat_updateToObs(obs, 0.001^2, 1.00, 1.0 );%(value, var, inflationFactor) 
end
toc
%*********************
...
plot(tt.double/3600, values,'-r' ); hold on
